...
class MovingAverageCross(BaseStrategy):
    ...
    def _calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Wilder EWMA, shared with the regime filter and risk sizer - see
        # quant/indicators.py.
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # No indicator warm-up yet -> no signal. The ATR guard matters
        # independently of the SMA one: take-profit here is an ATR multiple,
        # so a signal on a bar with NaN ATR would reach RiskManager as a
        # non-finite tp offset and raise. With the default periods the slow
        # SMA warm-up (50) already covers ATR's (14), but the periods are
        # constructor arguments and nothing stops atr_period > slow_period.
        signal[df["sma_slow"].isna()] = 0
        signal[df["atr"].isna()] = 0
