...
- **Dashboard** (`views/Dashboard.tsx`) — today's actionable setups (`GET /api/v1/signals/live-today`) alongside the current market regime badge (`GET /api/v1/market/regime`).
...
- **Historical Simulator** (`views/Simulator.tsx`) — replay a past date's scan (`POST /api/v1/backtest/historical-date-scan`) and simulate trade execution against it (`POST /api/v1/backtest/simulate-trade-execution`, `GET /api/v1/backtest/bars`).
