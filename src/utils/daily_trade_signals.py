...
def scan_for_signals():
    ...
    for ticker in tickers:
        ...
        if signal == 1 and not pd.isna(rsi) and rsi < 85:
            ...
            signals.append({
                ...
                'signal_strength': 'FRESH' if prev['st_signal'] == 0 else 'CONTINUING',
                'volume_trend': 'UP' if latest['Volume'] > df['Volume'].tail(20).mean() else 'DOWN',
    ...
    for rank, sig in enumerate(signals[:5], 1):
        ...
        print(f"      Risk/Reward: ~2.5x (Risk £{(sig['entry_price'] - sig['stop_loss']):.2f} for £{(sig['target_1'] - sig['entry_price']):.2f})")
        print()
